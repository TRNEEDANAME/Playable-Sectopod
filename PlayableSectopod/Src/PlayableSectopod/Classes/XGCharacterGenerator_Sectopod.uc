class XGCharacterGenerator_Sectopod extends XGCharacterGenerator
	dependson(X2StrategyGameRulesetDataStructures);

var localized string SectopodFirstName;
var localized string SectopodLastNamePrefix;
...
function GenerateName( int iGender, name CountryName, out string strFirst, out string strLast, optional int iRace = -1 )
{
	...
	strFirst = SectopodFirstName;
	strLast = SectopodLastNamePrefix;
	SparkIndex = 1;
	bUseRandomNum = true;
	...
	if(History != none)
	{
		...
		if(XComHQ != none)
		{
			...
			for(idx = 0; idx < XComHQ.Crew.Length; idx++)
			{
				...
				if(UnitState != none && UnitState.GetMyTemplateName() == 'PA_Sectopod')
				{
					SparkIndex++;
				}
			}
